...
module increment_time(
	...
	always@(posedge clk or negedge rst_n) begin 
		if (!rst_n) begin
			...
			// loading the inc hrs and min with the running time so it can be incremented from the current running time
			inc_hours    <= running_hours;
			inc_minutes  <= running_minutes;	
		end else if (set_alarm_time_pe) begin
			// loading the inc hrs and min with the alarm time so it can be changed from the current alarm time
